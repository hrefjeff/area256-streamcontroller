...
function init(){
	...
	function scoreboard(){
		...
		if(startup == true){
			...
			if(game == 'BBTAG' || game == 'SFVAE' || game == 'TEKKEN8' || game == 'UNIST'){
				$('#scoreboardVid').attr('src','../webm/scoreboard_1.webm');
			}
			else if(game == 'SF6' || game == 'DBFZ' || game == 'GGXRD' || game == 'KOFXIV' || game == 'MVCI' || game == 'UMVC3'){
	...
	function getData(){
		...
		var p1Controller = scObj['p1controller'];
		var p2Controller = scObj['p2controller'];
		...
		if(startup == true){
			...
			$("#p1TFlag").attr("src","../imgs/teams/"+p1Controller+".png").on("error",function(){
				$("#p1TFlag").attr("src","../imgs/teams/DEF.png");
			});
			
			$("#p2TFlag").attr("src","../imgs/teams/"+p2Controller+".png").on("error",function(){
				$("#p2TFlag").attr("src","../imgs/teams/DEF.png");
			});
			...
			if($('#p1Name').text() != p1Name || $('#p1Team').text() != p1Team){ //if either name or team do not match, fades out wrapper and updates them both
				TweenMax.to('#p1Wrapper',.3,{css:{x: p1Move, opacity: 0},ease:Quad.easeOut,delay:0,onComplete:function(){ //uses onComplete parameter to execute function after TweenMax
					...
					$('#p1Team').html(p1Team);
					$("#p1TFlag").attr('src','../imgs/teams/'+p1Controller.toUpperCase()+'.png').on("error",function(){
						$("#p1TFlag").attr("src","../imgs/teams/DEF.png");
					});
			...
			if($('#p2Name').text() != p2Name || $('#p2Team').text() != p2Team){
				TweenMax.to('#p2Wrapper',.3,{css:{x: p2Move, opacity: 0},ease:Quad.easeOut,delay:0,onComplete:function(){
					...
					$('#p2Name').html(p2Name);
					$('#p2Team').html(p2Team);
					$("#p2TFlag").attr('src','../imgs/teams/'+p2Controller.toUpperCase()+'.png').on("error",function(){
						$("#p2TFlag").attr("src","../imgs/teams/DEF.png");
					});
